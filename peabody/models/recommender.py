...
class Recommender:
    ...
    def get_recommendations(self, title: str, results: int, genre):
        ...
        if genre != "None":
            ...
            for index, score in sim_scores:
                if genre in self.df.loc[index]["Genre(s)"]:
                    book_indices.append(index)
            if idx in book_indices:
                book_indices = book_indices[1:results+1]
            else:
                book_indices = book_indices[0:results]
